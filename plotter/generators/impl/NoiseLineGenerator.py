...
class NoiseLineGenerator(Generator):
    ...
    def __init__(self):
        self.config = {"width": 16640, "height": 10720}
        super().__init__()
    ...
    def get_param_list(self) -> ParamList:
        return {
            ...
            "start_x": (
                "sx",
                "Starting location on x axis for drawing lines",
                "int",
                0,
                16640,
            ),
            "start_y": (
                "sy",
                "Starting location on y axis for drawing lines (ending location depends on number of lines)",
                "int",
                0,
                10720,
            ),
            "end_x": (
                "ex",
                "Ending location on x axis for drawing lines (as distance from border of region)",
                "int",
                0,
                16640,
            ),
    ...
    def get_default_params(self) -> ParamValues:
        return {
            ...
            "line_x_sin_freq": 50,
            "line_x_rand_amp": 50,
            "line_x_sin_amp_exp": 0.4,
            "line_x_sin_freq_exp": 0.4,
            "line_x_rand_amp_exp": 0.4,
            "line_y_sin_amp": 100,
            "line_y_sin_freq": 50,
            "line_y_rand_amp": 50,
            "line_y_sin_amp_exp": 0.4,
            "line_y_sin_freq_exp": 0.2,
            "line_y_rand_amp_exp": 0.4,
        }
    ...
    def generate_lines(self, **kwargs):
        ...
        for i in range(num_lines):
            cum_translation_y += round(
                ...
                + kwargs["line_distance_sin_amp"] * math.sin(i * (2 * math.pi) / 20)
            )
        ...
        for i in range(num_lines - 1):
            ...
            lerped_line = self.lerp_lines(
                line_map[current_unique_index],
                line_map[next_unique_index],
                fraction_till_next_line,
            )
        ...
        for i in range(num_lines):
            line = self.filter_oob(
                sample_spline(line_map[i], 1000, kwargs["spline_tightness"])
            )
            pen_num = 0
    ...
    def filter_oob(self, line: list[Py5Vector]) -> list[Py5Vector]:
        ...
        for v in line:
            if (
                v.x < 0
                or v.x >= self.config["width"]
                or v.y < 0
                or v.y > self.config["height"]
            ):
                continue
            new_line.append(v)
        return new_line
    ...
    def lerp_lines(
        self, line: list[Py5Vector], new_line: list[Py5Vector], ratio: float
    ) -> list[Py5Vector]:
        lerped_line = []
    ...
    def translate_line(
        self, line: list[Py5Vector], x_offset: int, y_offset: int
    ) -> list[Py5Vector]:
        new_line = []
        for vector in line:
            new_line.append(Py5Vector(vector.x + x_offset, vector.y + y_offset))
        return new_line
