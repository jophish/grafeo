...
class Repl:
    ...
    def set_generator(self, gen_num):
        ...
        self.current_generator.set_param_values(
            load_config()[self.current_generator.get_name()]
        )
    ...
    def set_parameter(self, param_info):
        ...
        for param_name, param_info in param_list.items():
            if param_info[0] == param_info_words[0]:
                try:
                    self.current_generator.set_param_value(
                        param_name, param_info[2](param_info_words[1])
                    )
                except:
    ...
    def plot(self):
        if not self.current_generator:
            print(
                "no generator currently defined. set a generator and generate something first"
            )
            return
    ...
    def refresh(self):
        ...
        config = load_config()
        self.current_generator.set_param_values(
            config[self.current_generator.get_name()]
        )
