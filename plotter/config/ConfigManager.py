...
CONFIG_PATH = os.path.join(Path(os.path.abspath(__file__)).parent.parent.parent.absolute(), CONFIG_FILENAME)
    ...
    def load_config(self):
        ...
        for generator_name in self.generator_defaults:
            if generator_name not in self.config['generator_params']:
                self.config['generator_params'][generator_name] = self.generator_defaults[generator_name]
