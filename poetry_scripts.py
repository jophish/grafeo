...
def test() -> None:
    execute(
        "tests",
        ["pytest", "tests", "--cov-fail-under", "100"],
...
def grafeo() -> None:
    execute(
        "grafeo",
        ["python3", "-m", "grafeo.main"],
        "Error running grafeo"
    )
def format_check() -> None:
    execute(
        "format",
        ["isort", "--check-only", "."],
        'Import ordering incorrect. Run "poetry run isort ." to fix.',
    )
    execute(
        "format",
        ["black", "--check", "."],
        'Running "poetry run black ." will fix most issues.',
    )


def format_fix() -> None:
    execute(
        "format", ["isort", "."], "isort formatting failed. Address issues manually."
    )
    execute(
        "format", ["black", "."], "black formatting failed. Address issues manually."
    )
...
def type_check() -> None:
    execute(
        "type_check",
        ["mypy", "."],
        'Mypy check failed. See above for errors, run "poetry run mypy ." to confirm fix.',
    )
